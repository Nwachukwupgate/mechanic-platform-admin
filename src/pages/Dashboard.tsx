...
import { adminAPI, type AdminStats, type ActivityItem } from '../services/api'
...
function activityHref(item: { kind: string; entityType?: string; entityId?: string }): string | undefined {
  if (item.kind === 'booking' || item.kind === 'quote' || item.kind === 'invoice')
    return item.entityId ? `/bookings/${item.entityId}` : undefined
  if (item.kind === 'transaction') return item.entityId ? `/transactions/${item.entityId}` : undefined
  if (item.kind === 'report') return item.entityId ? `/reports/${item.entityId}` : undefined
  if (item.kind === 'paystack') return item.entityId ? `/webhooks` : undefined
    ...
    <>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/audit" className="text-xs font-medium text-violet-700 hover:underline">
            Audit log →
          </Link>
        </div>
