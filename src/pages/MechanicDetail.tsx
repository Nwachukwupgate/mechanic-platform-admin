...
export default function AdminMechanicDetail() {
  ...
  return (
    <>
      <AdminPageHeader backTo="/mechanics" title={m.companyName} subtitle={m.ownerFullName} />
      ...
      <AdminSection title="Recent bookings">
        <ul className="text-sm space-y-1">
          {m.bookings?.map((b: any) => (
            <li key={b.id}><AdminLink to={`/bookings/${b.id}`}>{b.fault?.name} · {b.status}</AdminLink></li>
          ))}
        </ul>
      </AdminSection>
